...
class AvatarHandler(BaseHandler):
    ...
    def post(self):
        ...
        return self.write(dict(errno=RET.OK, errmsg="保存成功", data="%s" % image_url))
...
class NameHandler(BaseHandler):
    ...
    def post(self):
        ...
        if name in (None, ""):
            return self.write(dict(errno=RET.PARAMERR, errmsg="params error"))
        ...
        try:
            ...
            logging.error(e)
            return self.write(dict(errno=RET.DBERR, errmsg="name has exist"))
...
class AuthHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            # 数据库查询出错
            logging.error(e)
            return self.write(dict(errno=RET.DBERR, errmsg="get data failed"))
        logging.debug(ret)
        if not ret:
            return self.write(dict(errno=RET.NODATA, errmsg="no data"))
        self.write(dict(errno=RET.OK, errmsg="OK", data={"real_name":ret.get("up_real_name", ""), "id_card":ret.get("up_id_card", "")}))
    ...
    def post(self):
        ...
        try:
            ...
            logging.error(e)
            return self.write(dict(errno=RET.DBERR, errmsg="update failed"))
        self.write(dict(errno=RET.OK, errmsg="OK"))